...
class StreamingService {
  ...
  private async initWs(
    ...
    const base =
      (import.meta as any).env?.VITE_SERVER_WS_URL ||
      (typeof window !== "undefined"
        ? `${window.location.protocol === "https:" ? "wss" : "ws"}://${
            window.location.host
          }`
        : "");
    const wsUri = `${base}/?id=${id}&device=${options.device}&audio=${
      options.audio
    }${
      options.audio
        ? `&audioCodec=${options.audioCodec}&audioEncoder=${options.audioEncoder}`
        : ""
    }&video=${options.video}${
      options.video
        ? `&videoCodec=${options.videoCodec}&videoEncoder=${options.videoEncoder}&maxFps=${options.maxFps}&videoBitRate=${options.videoBitRate}`
        : ""
    }`;
